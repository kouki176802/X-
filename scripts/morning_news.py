...
def build_prompt(items: list[NewsItem], limit: int) -> str:
    ...
    return textwrap.dedent(
        ...
        出力形式:
        【朝のニュース心理Xスレッド案】YYYY-MM-DD

        カテゴリ｜ニュースタイトル
        記事: URL

        投稿1:
        180〜260字。ニュース要点を濃く書き、読者が反応する感情（不安、驚き、共感、損失回避、違和感など）を入れて強く引く。最後は必ず「理由は↓」「原因は↓」「心理的には↓」など、リプ欄を読みたくなる一文で終える。

        投稿2:
        180〜260字。心理学・行動心理学の概念を1つ入れ、今回のニュースで人がなぜ反応するのかを具体的に説明する。用語の説明だけで終わらせず、読者の身近な感情に接続する。

        投稿3:
        180〜260字。SNSで話題化しやすい理由を書く。怒り、共感、不安、意外性、損失回避、権威バイアスなど、拡散される心理を具体化する。

        投稿4:
        180〜260字。読者が今日から使える見方、注意点、判断のコツを書く。抽象論ではなく「一度止まる」「事実と感情を分ける」など行動に落とす。

        投稿5:
        必要な場合だけ120〜220字で補足を書く。事件・犯罪では配慮、経済では冷静な判断、エンタメでは人間関係や応援心理に接続する。

        口調パターン:
        以下の10パターンを、記事内容に合わせてランダムまたは適切に使い分ける。同じ日の10件で同じ始まり方を連発しない。
        - 冷静な解説者系: 「このニュース、表面だけ見ると〇〇ですが、心理的には別の動きがあります。」
        - ちょっと鋭い考察系: 「たぶん多くの人が引っかかるのは、ニュースの中身より“そこにある違和感”です。」
        - やさしい先生系: 「この話題は、怒る前に少しだけ心の仕組みで見てみるとわかりやすいです。」
        - フック系: 「これ、ただのニュースとして流すのは少しもったいないです。」
        - 雑談っぽい自然体系: 「このニュースを見て、少し考えたことがあります。」
        - 心理学ラボ系: 「人がこの話題に反応する理由を、行動心理学で分解してみます。」
        - ニュースの裏側系: 「このニュースで本当に面白いのは、出来事そのものより人の反応です。」
        - 共感スタート系: 「これを見てモヤッとした人、多いと思います。その感覚には理由があります。」
        - 注意喚起系: 「こういうニュースを見る時に、ひとつ気をつけたいことがあります。」
        - 物語っぽい系: 「ニュースは事実を伝えます。でも、人が動くのは事実より“感じ方”の方です。」

        ルール:
        - 記事URLは必ず残す
        - 本文内で「①②③④⑤」「1. 2. 3.」のような番号を使わない
        - Discordで確認しやすくするため、ラベルは「投稿1:」「投稿2:」のようにする
        - 投稿本文は人が書いたように自然な接続語でつなぐ。「まず」「ここで働くのが」「さらにSNSでは」「だから見る側は」などを使う
        - 全投稿で同じ語尾や同じ構文を繰り返さない
        - 事件・犯罪では憶測、断定、個人攻撃、過度な煽りを避ける
        - 犯罪・事件では被害者や関係者への配慮を入れ、犯人像の決めつけをしない
        - 「心理学的に見ると」だけで終わらせず、具体的な心の動き、認知バイアス、行動変化を書く
        - 読者に不安を売る文章ではなく、ニュースを冷静に見るための知識として書く
        - 文章は硬すぎず、Xで読まれるテンポにする。1投稿ごとに読み切れる密度にする
        - 長文1本にはしない。リプ欄へ自然に誘導するスレッド形式にする
        - Discordで読みやすいように簡潔に
...
def generate_with_openai(prompt: str) -> str:
    ...
    payload = {
        "model": model,
        "input": prompt,
        "instructions": "あなたは心理学と行動心理学に詳しい日本語SNS編集者です。ニュース事実を尊重し、冒頭で惹きつけてリプ欄まで読ませるXスレッド案を作ります。",
    }
